...
```bash
uv run python scripts/generate_demo_mf4.py --profile smoke   # CI/開発用: 10秒・数MB
uv run python scripts/generate_demo_mf4.py --profile quick   # 機能確認用: 5分・約180MB
uv run python scripts/generate_demo_mf4.py --profile hils    # 本番相当: 60分・約2GB
```

- `--out PATH`（既定 `demo_data/hils_demo.mf4`）・`--duration SEC`（プロファイル既定秒数の上書き）・
  `--seed INT`（既定42・同一シードで再現可能）・`--dirty`（`VehDyn_10ms` グループ (CAN) に
  重複/非単調タイムスタンプを注入し、LD-03/04 の warning 診断を発火させる）。
- 生成物は `demo_data/`（`.gitignore` 済み）配下に置く運用とし、コミットしない。
- 所要時間/サイズは spec §4.1 の概算値（実測ではない・生成時間はマシン依存）。`hils` プロファイルは
  XCP 1ms 高速ラスタが主要因で、実測してみてズレが大きい場合は `--duration` で調整する。
...
- `--dirty` を付けると `VehDyn_10ms`（CAN: `VehSpd` 等）に重複/非単調タイムスタンプが混入し、
  Diagnostics ドックに LD-03/04 の warning（「非単調 N 箇所・重複タイムスタンプ M 点」）が表示される。
