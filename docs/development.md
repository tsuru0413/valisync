...
FU-03（多チャンネルのドック切替フリーズ）を実機再現するための大規模データ。（FU-01「1024列超の展開ダイアログがスクロール不能」も当初の目的だったが、**E-3 でダイアログごと退役**したため対象外 — 広幅アレイは確認なしで載る。）
...
- **実測値（2026-07-10・Windows 11 / AMD64・`uv run` 経由・seed 42）**: 生成 **25秒**（24チャンク）/ ロード **Session.load 8.0s＋materialize 2.2s ≈10秒** / 実サイズ **1.36 GB**（推定 1.359GB とほぼ一致＝差は mf4 メタデータ）/ 展開後の**列**数 **330,004**（1024 ガード時代は広幅 60×1100=66,000 をヘッドレスで全スキップし 264,004 列だった）。いずれも目標 1.5GB/32万/120s の±20%内。
- **E-3 反転後の実測（2026-07-29）**: 1024 ガードは退役し広幅も普通に載るので列は **330,004** 全数。`Signal` オブジェクトは **物理チャンネル 4,324 個**（列は要求時に鋳造）で、ロードは **core 1.43 s / 実 GUI 2.40 s**。「ch 数」を語るときはどちらの単位かを必ず添えること（ブラウザのヘッダー母数と `source_info.n_channels` は U2 の決定どおり**列**単位）。
