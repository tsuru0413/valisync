...
- **実測値（2026-07-10・Windows 11 / AMD64・`uv run` 経由・seed 42）**: 生成 **25秒**（24チャンク）/ ロード **Session.load 8.0s＋materialize 2.2s ≈10秒** / 実サイズ **1.36 GB**（推定 1.359GB とほぼ一致＝差は mf4 メタデータ）/ 展開後 ch 数 **330,004**（うち広幅 60×1100=66,000 はヘッドレスで全スキップ→**264,004 ロード**。E-3 で 1024 ガードを退役したため現在の数値は T9 の再実測で更新）。いずれも目標 1.5GB/32万/120s の±20%内。
