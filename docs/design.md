...
| R-07 | **助数詞**（UX-11）: 診断・ヒット件数=「件」／信号総数=「N 信号」・フィルタ結果=「{n} 信号中 {m} 件を表示」の単一パターン／展開系列・チャンネル=「本」／列=「列」／サンプル=「点」／非単調=「箇所」。「ch」はファイル情報等の技術メタ情報に限定。数詞は半角数字＋前後半角スペース（「1信号」直結は禁止）。**ただしチャンネルブラウザの件数表示のみ、展開列を「ch」単位で数える（`{total} ch 中 {shown} ch を表示`）— ユーザー決定 4・列展開遅延化 E-2。R-07 の単一パターン規定と「ch は技術メタ情報に限定」を当該箇所に限り supersede**（対訳表 G-42 と対で記録・[deferred-column-expansion spec](superpowers/specs/2026-07-24-deferred-column-expansion-design.md)）。 |
...
- 2026-07-25: チャンネルブラウザの件数を**展開列の「ch」単位**へ（トークン変更なし・
  文言のみ）。ブランチ `feature/lazy-load-samples`（E-2「列展開の遅延化・ブラウザ」
  Task 4・PR 未採番）。`CHANNEL_HEADER_COUNT_TMPL` = `{total} ch 中 {shown} ch を表示`・
  `CHANNEL_HEADER_EMPTY_TMPL` = `0 ch`（**桁区切りは付けない** — 現行 `{total}` の
  挙動を保存・ユーザー決定 4）。**理由**: E-2 で木が 1 行 = 1 物理チャンネルへ平坦化
  され（子の列は展開時に合成）、件数が数える対象が「信号」から**展開列**へ変わった。
  行数（prod 4,324）と件数（同 264,004）は別単位なので、件数だけ「N 信号」のままだと
  読者が行と突き合わせられない。**表記規約の改訂を伴う**: R-07 の単一パターン
  「{n} 信号中 {m} 件を表示」と「ch は技術メタ情報に限定」を**当該箇所に限り
  supersede**（design.md R-07・対訳表 G-42・
  [incd-strings inventory](superpowers/specs/2026-07-22-incd-strings-inventory.md):309-310 に
  記録 — 特に inventory 行には「ch は技術メタ情報限定ゆえ ch→信号」という**逆向きの
  根拠**が書かれており、無記録だと次の文言スイープで確実に差し戻される）。
  **併せて記録するユーザー可視の挙動変更（コントローラ決定 2）**: グループ化キーが
  ローダー由来の `metadata['physical_channel']` になったため、**CSV/Derived の
  `Arr[0]`/`Arr[1]` は親行にならず個別行のまま**になる（`physical_channel` を持たない
  = 1 列 1 物理チャンネル）。CSV の `Arr[0]` は展開された列ではなく「そう名付けられた
  1 本のチャンネル」なので、名前の形だけを根拠に畳むと実体を偽ることになる。
  **同増分の perf 修正（文言ではないが同コミット）**: フィルタ 1 適用で 3 回走っていた
  列名走査（`tree_groups`／`header_text`→`shown_count`／`empty_state`→`shown_count`）を
  フィルタ文字列キーの **0 MB メモ**（`_filter_memo`）で 1 回に畳んだ。名前キャッシュ
  （≈33 MB）は E-3 の条件付きへ移送。`_matches` 呼び出し数の実測で 3.00 → 1.00 パス
