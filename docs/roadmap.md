...
>
> **E-4c（衝突診断＋AST 構造ガード）完了（PR #156）＝遅延ロードシリーズ全増分完結**: 残っていたサイレント破壊の穴 3 つを閉鎖（4 タスク subagent-driven・全体レビュー Ready-to-merge Critical/Important 0）。(1) **衝突 info 診断** — 2-D `Mat` と実在 1-D `Mat[0]` の共存で最長一致により配列側リーフが**到達不能**になる配置を、`load()` 末尾（プラン著者の実コード probe で「グループ末尾案は `column_records` がファイル全体アキュムレータのため append 順依存の無言取りこぼし」と判明し spec 修正）で静的検出し info 診断 1 件/ペア（既存 LoadOutcome plumbing・範囲外リーフ名は非診断・多段 `W[0][1]` 対応）。最長一致 walk を `owning_candidates` へ抽出し検出・解決の**規則を単一実装化**（解決経路はゼロ挙動変更を候補順序・短絡・reject 意味論の step 一致でレビュー確認・`has_column` は `test_column_roundtrip.py` 明文の独立オラクルとして意図的に不統合）。**到達不能性そのものの負 assert を初常設**（全物理チャンネル×全列名の resolve スイープと診断件数の突合 — 片側破れが必ず RED・境界 `<=` 変異も kill）。(2) **AST 構造ガード** — `dict(signal_map())` 系の全列列挙が `src/` に書かれた瞬間 CI が落ちる上段防御（検出形 4 種・ratchet allowlist 空・陳腐化 assert・レビュアー独立 probe で set/sorted/iter/tuple/内包の未演習変種まで検出確認・lookup 系 false positive ゼロ）。下段の既存挙動テストとは二層独立オラクル。alias 変数（名前に signal_map を含まない）は意図的ヒューリスティック限界として記録。(3) **`_evict_resolved` の lock 外解放**（親 spec §12-2・E-4b で export worker が `_resolved_lock` の取り手になり defer 根拠失効）— `_evict_oldest` 同型（lock 内 detach・呼び出し側が lock 退出後に解放）。観測子（Signal metadata 常駐 probe の `__del__` で lock 状態記録）で両呼び出し経路の honest-RED → GREEN、「probe 列非 evict で素通り」の空虚化ホールはレビューが newest-first 変異で実証（2 passed 盲目）→ `assert observed` 1 行で blind→detecting 反転。(4) docs 正誤（CacheKey serial 化の stale 記述是正）。フルスイート 2,483+・GUI ソース無変更のため realgui/凍結カタログ再撮影は不要（全体レビューが構造的根拠つき承認）。defer chip: interior-node 一致の診断 gap（レビュアーの実 mf4 probe で「asammdf がサブ配列を 1 段リーフに潰すため実ファイル経由では現状再現不可」と確定・E-4c spec §2）。spec: [2026-08-04-e4c-collision-diagnostics-ast-guard-design.md](superpowers/specs/2026-08-04-e4c-collision-diagnostics-ast-guard-design.md)、プラン: [2026-08-04-e4c-collision-diag-ast-guard.md](superpowers/plans/2026-08-04-e4c-collision-diag-ast-guard.md)。
>
