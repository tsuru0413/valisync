...
| FU-08 | 🔴 | **根因確定・実測（2026-07-10・prod 264k信号）— 当初仮説（isfinite min/max）は誤り**。真犯人は **`_signal_map()` が呼ばれるたび `session.signals()` の全 264,004 信号をループして `{name: signal}` 辞書を再構築**（split＋dict×264k）。**信号追加1本で ~8秒・`reset_y()` 100信号で ~5.4秒**（per-add はプロット数非依存の一定8s＝full-session rebuild が支配的）。対して **`finite_view()`（キャッシュ済）＋min/max は全100信号で 2.07ms** ＝ **~2600倍**の無駄。呼び出し元9箇所（`reset_y`/`reset_axis_y`/`reset_x`/`_auto_fit_ranges`＝**信号追加のたび**・他）。`render_data`(L787) も呼ぶが**レンダキャッシュ**で通常 paint は無影響（FU-03 が速い理由と整合）。**修正方向**: `_signal_map()` を**キャッシュ化**（session load/unload＋オフセット変更で invalidate）or **プロット中キーへスコープ**（session に by-name 参照が無いので groups 側に `{name:signal}` ベースマップを1回構築しオフセットのみ差分適用）＝根本 | `gui/viewmodels/graph_panel_vm.py:1220`（`_signal_map`）＋呼び出し元 `:643/706/725/1246` | **大容量データで信号を1本プロットするたび約8秒フリーズ**（コア操作が事実上使用困難） |
