...
| FU-16 | 🟠 真因確定 | 〔perf〕prod（330k ch）のクローズで確認ポップアップ表示まで・確認後のクローズ処理の両方が長い。**真因確定（2026-07-11・コード読解・前後切り分け／実測は着手時に prod で締める）**: 確認**前**は基本 light（`_close_selected`→`files[row]` キャッシュ＋QMessageBox 即時）。例外=**右クリック**で非選択行を閉じる時のみ `setCurrentIndex` が selectionChanged→`set_active_file`（同一キー dedup なし）→`ChannelBrowserVM` prep 再構築 O(該当 ch)。確認**後が真のボトルネック**: `prune_missing_signals`（`graph_panel_vm.py:490`）が `session.signals()` を**全タブ全パネル=N回**呼び、`remove()` の無効化直後の初回で残存全信号の namespaced 全再構築（残存データ比例）＋以降も毎パネル O(n) スキャン＝(i)一回再構築＋(ii)パネル数乗数の両方。**FU-11 の group_signals キャッシュは prune 非経由＝直交**。**根因フィックス**: `prune_missing_signals` を「生存**グループキー**集合（`SignalGroupManager.keys`＝O(#files)）でフィルタ」へ変更（チャンネル数非依存＋クローズ時の全再構築を発火させない）。副次: `_invalidate_namespaced` は削除キーのみ drop。ヘッドレス call-count 構造テスト（N×P 呼出→fix後0）で証明可 | `gui/views/file_browser_view.py:135-154`・`gui/viewmodels/app_viewmodel.py:118-141`・`core/loaders/signal_group_manager.py:43-54,103-146`・`gui/viewmodels/graph_area_vm.py:57-85`・`gui/viewmodels/graph_panel_vm.py:482-497`（prune・真のボトルネック L490） | 大容量ファイルのクローズ操作で長時間フリーズ |
