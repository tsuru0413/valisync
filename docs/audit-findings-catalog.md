...
| FU-03 | 🟡 要再現条件 | **現行コードで未再現**（cProfile 実測・2026-07-10）: prod（264k ch・1.36GB）ロード＋**100信号プロット（120万点）でもドック切替は ~44ms**（信号なし34ms／20信号38ms／100信号44ms・file/channel は9–12ms）。cProfile の hot path は pyqtgraph paint（`PlotCurveItem.boundingRect`/`dataBounds`/`nanmin`）で **valisync ダウンサンプラは非出現＝切替（幅リサイズ）は再描画のみで LOD 再ダウンサンプルを起こさない**。報告後マージの **RN-04（ダウンサンプラ・ベクトル化 ~53×）＋perf 一連**で緩和済みの公算大（断定は保留＝ハーネスは `processEvents` 駆動で実機の対話連打と完全一致せず）。**再開には実機の具体条件（プロット信号数/パネル・軸数/ズーム状態）が必要** | `gui/views/main_window.py:167`（View menu）・`:195`（ツールバー） | 大容量データでドック開閉が体感フリーズ（現行では再現せず） |
...
| FU-15 | 🟡 UX | 〔UX 欠落〕アクティブ Y 軸を**解除する手段がない**: `set_active_axis(None)` を渡す呼び出しがリポジトリに存在せず、解除は別軸/曲線クリックでの**置換のみ**（`_active_axis_index` は一度セットされると None に戻る経路なし）。Y 軸以外（パネル空白等）のクリックで解除されるようにしたい | `gui/views/graph_panel_view.py:463-481,728,732,1206-1218,1224-1235` | アクティブ軸の枠/操作モードを意図的に外せない |
...
| FU-18 | 🔵 未確認（コード読解仮説・e2e未検証） | 〔perf・調査中に浮上〕FU-16 の調査で Fable5 アドバイザーが指摘した別経路: `Session.source_info`（`session.py:232`）が 330k× `sorted_view` を構築し、ソース情報のツールチップ hover でフリーズし得る。**未再現の仮説**（FU-16 で静的読解仮説が実測棄却された教訓を踏まえ、断定しない）。着手時に実アプリ hover 経路で e2e 再現を締めてから真因確定扱いにする | `core/session.py:232`（`source_info`） | ソース情報の hover で大容量時にフリーズ（未検証） |
| FU-17 | 🟡 UX | 〔UX 小バグ〕「Sync X」チェックボックスのヒット判定が文字よりはるか右の余白まで広い: `QVBoxLayout.addWidget(sync_checkbox)` が alignment/stretch 未指定のため QCheckBox 本体がレイアウト幅いっぱいにストレッチされ、余白もクリック判定に含まれる。判定をチェックボックス＋ラベル上のみへ（`addWidget(cb, 0, Qt.AlignLeft)` 等で幅を内容に固定） | `gui/views/graph_area_view.py:104,130-133` | 余白の誤クリックで X 同期が意図せずトグルされる |
