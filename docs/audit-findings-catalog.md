...
| LD-10 | 🟡 | 大容量 MDF4 で配列多重コピー（astype＋Signal 再コピー）→ OOM リスク。**実測（2026-07-04・HILS デモ hils 2.01GB/171ch・Win11）: ロード 7.8 秒・プロセスピーク +7.3GB＝ファイルサイズの約 3.6 倍に膨張**（float64 変換＋`copy_master=True`＋read-only コピーの合算。quick 167MB では +0.66GB で同倍率） | `core/loaders/mdf4_loader.py:134` | 大きいログでメモリ不足（2GB 級ログで RAM 8GB 占有を確認 — 本番相当ファイルで実害域・第3弾の最優先候補） |
