...
| FU-11 | 🟠 | 〔perf〕ChannelBrowser フィルタが prod（330k ch）で**1文字入力ごとにフリーズ**。経路: `textChanged→set_filter→_notify("filter")` が同期で `SignalTableModel` を `beginResetModel`＋`list(vm.signals)` 全再構築。`signals` プロパティは毎回 `session.group_signals()`→`_namespaced()` を**キャッシュなしで再実行**（対象ファイル全チャンネルの Signal ラッパー新規生成・FU-08 の `signal_map()` キャッシュの対象外）＋全件線形 `.lower()` 比較。さらに `_refresh_state` の `header_text()`/`empty_state()` が独立に `signals` を再評価し**1キーで最大3回**この経路が走る。**要件: デバウンス/ディレイでなく可能な限りリアルタイム更新を維持**（`group_signals` のキャッシュ化・軽量 name-only 走査・インクリメンタル絞込等の根本策を優先） | `gui/views/channel_browser_view.py:125,161-164`・`gui/viewmodels/channel_browser_vm.py:63-90,106-125,139-142`・`gui/adapters/qt_signal_models.py:107-111`・`core/loaders/signal_group_manager.py:74-100,125-130` | 大容量ファイルでフィルタが実用にならない（1文字ごとフリーズ） |
| FU-12 | 🟠 | 〔描画の正しさ〕`Radar.Obj0.dx` を新規パネルに追加すると t=0-60 の定数区間 y=45 が Y レンジ下限と一致してプロット境界に張り付き視認できない。根因候補: `_padded_range` は**軸全体の集約 min/max が縮退（ほぼゼロ幅）のときのみ** ±50% パディング（RN-05）で、部分定数区間の値が全体 min/max と一致するケース（他区間に変動あり＝非縮退）は素通し（`return (lo, hi)`）でパディングされない | `gui/viewmodels/graph_panel_vm.py:49-61`（`_padded_range`）・`:625-632,634-663,1246-1296`（fit 経路） | レンジ境界と一致するデータ区間が見えない |
...
| FU-16 | 🟠 | 〔perf〕prod（330k ch）のクローズで確認ポップアップ表示まで・確認後のクローズ処理の両方が長い。調査（2026-07-11）: 確認**前**は軽い（`vm.files` はキャッシュ O(1)＋QMessageBox 即時）— 遅延の真因候補は確認**後**: `"unloaded"` 通知→`GraphAreaVM._on_app_change`→**全タブ全パネル** `prune_missing_signals()` が `session.signals()` を呼び、`remove()` が破棄した namespaced キャッシュを**残存全ファイル分 GUI スレッド同期で再構築**（FU-08 同族）。「確認前も遅い」体感は要実測（別要因の可能性） | `gui/views/file_browser_view.py:110-133`・`gui/viewmodels/app_viewmodel.py:123-141`・`core/session.py:250-266`・`core/loaders/signal_group_manager.py:43-54,107-123`・`gui/viewmodels/graph_area_vm.py:57-67`・`gui/viewmodels/graph_panel_vm.py:482-497` | 大容量ファイルのクローズ操作で長時間フリーズ |
