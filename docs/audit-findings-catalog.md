...
バケット②6サブスペック完了後、実使用で見つかった課題。**FU-07 完了（PR #77・下記）、他は未着手。** 着手時は該当箇所を再確認のうえ `brainstorming`→`writing-plans`。FU-01〜04 は不具合（FU-03/04 は根因が仮説段階＝要プロファイル/再現条件特定）、FU-05/06 は既存ボタン（SH-07/PC-02 で追加）の廃止 UX リファイン要望（他の開く/追加導線あり・廃止で発見性を失わないか確認）。FU-07 はデモデータ tooling 強化（本番想定 mf4・アプリ不具合ではない）＝**完了（PR #77・`prod` プロファイル追加）。これで FU-01 をヘッドレスで再現確定**（実 prod の広幅60本→`ExpansionDialog` 全高1940px＞画面816px・スクロール無し・OK/Cancel 画面外）。FU-08 は perf 不具合、FU-09 は右クリックにズーム/引きを追加する UX 要望。
...
| FU-02 | 🟡 | 大容量ロード中の `BusyOverlay` が `cover()` 実行時のみ `parent.rect()` へリサイズし、Diagnostics 等ドックの表示切替で親領域が変わっても追従しない（親の resize を購読していない）→ 進捗/キャンセルが本来位置とズレて重なる | `gui/views/busy_overlay.py:56-59` | ロード中にドック切替すると「読み込み中/キャンセル」表示が被る |
| FU-03 | 🟠 | 大容量ファイル読み込み済み状態でドック表示切替（`toggleViewAction`）がフリーズ気味・遅い。切替に伴う relayout がグラフエリアの再描画（LOD）を誘発している疑い（要プロファイル） | `gui/views/main_window.py:167`（View menu）・`:195`（ツールバー） | 大容量データでドック開閉のたびに体感フリーズ |
