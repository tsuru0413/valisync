...
## covered — 優先度 low（Plan 7 で解消）

- **ドロップ青枠ハイライト（ドラッグ enter/leave・ドロップ可能枠）** → **covered** `test_signal_dnd_realclick.py::test_drop_highlight_visible_mid_drag`（Plan 7・mid-drag `is_drop_highlighted` assert）
- **非アクティブ軸 hover 仮フレーム** → **covered** `tests/realgui/test_axis_hover_frame.py`（Plan 7・`_hover_axis_index` assert）
- **grip_hit_area_grabbability** → **covered**（既存 resize/zoom/move realgui が具体点掴みで実質カバー済み・新規不要・記録のみ）
- **DataExplorer OS ファイルドロップ** → **covered** `tests/realgui/test_data_explorer_file_drop.py`（Plan 7・URL-mime substitute＋load_handler spy）
...
- **C3 covered の caveat**: `test_move_then_resize.py::test_first_resize_after_axis_move_works` は load-bearing assert が描画ジオメトリでなく VM `height_ratio`＝②的に borderline（描画ジオメトリ assert への昇格余地）。→ **昇格済（Plan 7）**: `test_move_then_resize.py` が rendered viewbox `sceneBoundingRect().height()` 縮小を追加 assert し、描画ジオメトリ検証へ昇格。
