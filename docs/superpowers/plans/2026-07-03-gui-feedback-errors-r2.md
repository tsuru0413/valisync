...
- [ ] **Step 4: Implement — mdf4_loader.py**
...
- [ ] **Step 5: Implement — csv_loader.py**
...
```python
            # 1000 行ごとの協調的キャンセル確認（毎行だとオーバーヘッド・spec §4.1）
            if cancel is not None and raw_idx % 1000 == 0 and cancel():
                raise LoadCancelled(f"load cancelled: {file_path.name}")
```

- [ ] **Step 6: Run tests / full suite / gates**
...
- [ ] **Step 3: Implement — busy_overlay.py**
...
- [ ] **Step 4: Implement — load_worker.py**
...
- [ ] **Step 3: Implement — channel_browser_vm.py**
...
- [ ] **Step 3: Implement — channel_browser_view.py**
...
- [ ] **Step 4: Run tests / 全 suite / gates**
...
- [ ] **Step 3: Implement — file_browser_view.py**
...
- [ ] **Step 3: Implement — file_browser_vm.py**
...
- [ ] **Step 4: Implement — qt_signal_models.py（FileListModel.data）**
...
- [ ] **Step 2: scoped 実行で動作確認（マウス数秒占有）**

Run: `uv run pytest --realgui tests/realgui/test_busy_cancel_realclick.py -v`
Expected: PASS（フレークする場合は processEvents 回数・待機を調整）。

- [ ] **Step 3: `/gui-verify` で①証拠ゲート**

- [ ] `uv run pytest --realgui tests/realgui/test_busy_cancel_realclick.py`（該当のみ scoped）pass ログ＋スクショ証拠を添付
- [ ] headless full `uv run pytest` が 0 errors
- [ ] 既存 realgui の回帰: 本弾は既存入力経路無変更だが、`channel_browser_view.py` を触るため `uv run pytest --realgui tests/realgui/test_channel_browser_realclick.py` と信号 D&D `test_signal_dnd_realclick.py` を回帰実行
- [ ] CI 緑（push 後確認）
