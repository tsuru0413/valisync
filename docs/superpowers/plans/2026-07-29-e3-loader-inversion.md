...
数値列キーは **36 本**、非数値リーフは 1 本（`Mix.tag`）、仮想グループは **12 個**。
**手計算せず必ず measure して貼ること** — この母数は過去 2 回とも手計算が外れている
（28 と主張 → レビューで 26 → 実測 29、そして Mat2 追加後の実測が 36）。
再測定は fixture writer とオラクルを**テストモジュールから import して**行う:
`PYTHONPATH=. uv run python -c "..."` で `_write_roundtrip_mf4` → `_oracle_columns` →
`sorted(_numeric(...))` を印字し、その出力をそのまま `_EXPECTED_COLUMN_KEYS` へ貼る。

`Mat2`（同名 2-D チャンネル 2 本・列数 3 と 4）は **selector 側の契約を固定する唯一の腕**。
`dup` は 1-D なので selector について何も言わず、他の展開チャンネルは名前が一意なので
表示名 = 生名になってしまう。「dedup される展開チャンネル」だけが表示キー（`Mat2[0][i]`）と
生名由来 selector（`Mat2[i]`）を字面から食い違わせる。実測: この腕が無い状態
（e48e72a 版オラクル 10 本）では、selector を表示キーから導く sabotage が **10 passed =
完全に盲目**だった。
...
T0 は production を変更しないので「未実装ゆえの RED」は存在しない。代わりに**オラクル自身の honest-RED をここで先に見る** — `_flatten` に 1 文字の非対称を入れる（**編集先は Step 5 冒頭の隔離コピー**。他作業が並走する worktree で `src/` を直に編集してはならない）: `src/valisync/core/loaders/mdf_loader.py:71-75`（**ソース編集なのでオラクル側の `_flatten` も同時に壊れる**＝最も検出しにくい対称的破壊）:
...
Expected: FAIL — 実測で **5 failed / 6 passed**。
...
FAILED ...::test_deduplicated_exploded_channel_pins_both_name_spaces - AssertionError: Mat2[0][0]
...
**この 5 本が RED になる／全数 2 本（`test_every_numeric_column_key_resolves_with_matching_values`・`test_metadata_matches_oracle_for_every_column`）は GREEN のままである**ことを必ず目視する。全数ループは参照側も `_flatten` を使うため対称的破壊に構造的に盲目であり、ハードコードしたキー列（腕の代表・手書き期待値・`Nest.g[i]` の実測）だけが防波堤になっている — この非対称を理解せずに「全数があるから安心」と読むのがこのファイル最大の誤読ポイント。
...
| 契約 | 固定するテスト |
|---|---|
| 列キー 36 本すべてが `Session.resolve_signal` で解決でき、値・dtype・時刻列が一致 | `test_every_numeric_column_key_resolves_with_matching_values` |
...
| LD-08 dedup 表示名（`dup[0]`/`dup[1]`） | `test_ld08_duplicate_names_resolve_to_distinct_columns` |
| **二重名の selector 側**（dedup × 展開 = `Mat2`。列キーは `Mat2[0][i]`／selector は生名由来の `Mat2[i]`・生 selector 空間 `Mat2[i]` は表示空間へ漏れない・列数 3/4 の相違） | `test_deduplicated_exploded_channel_pins_both_name_spaces` |
...
```
Expected: `11 passed`（実測 1.1s。fixture は module-scope で mf4 書き出しとオラクル構築を 1 回だけ行う）。

母数の実測値（`_EXPECTED_COLUMN_KEYS` の裏付け）: 数値列キー **36 本** / 非数値リーフ **1 本**（`Mix.tag`, dtype `S2`）/ 仮想グループ **12 個**。うち 6 本（`Radar.ObjList[0..2]` / `Radar.ObjList.dx[0..2]`）は asammdf が CABLOCK 配列に対して実在させる**要素チャンネル**（展開列ではない）、7 本（`Mat2[0][0..2]` / `Mat2[1][0..3]`）は dedup 済み表示名の下に生えた展開列。`demo_data/` に一切依存しないので CI で実行される。
...
**sabotage の実施場所について（重要）**: `src/` を直接編集して `git checkout --` で戻す手順は、
同一 worktree で他の作業が並走しているときに**他人の未コミット変更を消す**。安全な代替として、
**committed source の隔離コピー**に対して sabotage を当て、`PYTHONPATH` でそちらを先に読ませる:

```bash
SC=<scratchpad>; git archive <task0-commit> src | tar -x -C "$SC/sabo"
PYTHONPATH="$SC/sabo/src" uv run pytest tests/core/loaders/test_column_roundtrip.py -q
# sabotage は $SC/sabo/src 配下だけを編集する (リポジトリの src/ は一切触らない)
```
`import valisync` は PYTHONPATH が editable install より先に解決されることを実測で確認済み。
下記 (a)〜(f) の実測値はすべてこの隔離ハーネスで採った。

**(a) `_flatten` の索引トークン非対称（Step 2 で実施済み・再掲）** — `mdf_loader.py:71-75` の `f"{name}[{i}]"` → `f"{name}({i})"`。実測: **5 failed / 6 passed**（上記のエラー文）。ソース編集なので参照側も同時に壊れる最悪ケースであり、それでも RED になることを確認済み。

**(b) LD-08 dedup を殺す（オラクルの独立性の実証）** — `MdfLoader._count_names` の戻りを全件 1 にすると、production の表示名が `dup` / `Mat2` に潰れて `dup[0]`/`dup[1]`/`Mat2[j][i]` が解決不能になる。実測: **6 failed / 5 passed** — `test_every_numeric_column_key_resolves_with_matching_values` / `test_group_master_is_shared_by_identity` / `test_metadata_matches_oracle_for_every_column` / `test_literal_values_pin_the_key_to_column_mapping` / `test_ld08_duplicate_names_resolve_to_distinct_columns` / `test_deduplicated_exploded_channel_pins_both_name_spaces`。

> **ロスターの訂正（T6 の実施者向け）**: 初版のプランはこの (b) で `test_oracle_covers_every_naming_arm` も RED になると書いていたが、**実測では GREEN のままである**。理由は 1 行で言える — このテストは `_numeric(loaded.columns)`（= オラクル側だけ）を assert しており、オラクルは自前の `Counter` で dedup を独立に計算するので、production の dedup を殺しても動かない。**これは壊れているのではなく、オラクルが本当に production から独立していることの直接証拠**。予測との差分を「何か壊れた」と読まないこと。（Mat2 追加前の実測は 5 failed。追加後は `test_deduplicated_exploded_channel_pins_both_name_spaces` が加わって 6 failed。）

**(c) master identity を殺す** — `mdf_loader.py:472-473` の `master` を列ごとにコピーする（`writeable=False` を保ったコピーにすること — 素の `.copy()` だと `flags.writeable` の assert が先に落ちて identity の歯を実証できない）と `test_group_master_is_shared_by_identity` が `assert sig.timestamps is ref.timestamps` で RED。実測: **1 failed / 10 passed**。

**(d) 数値ゲートを殺す** — `mdf_loader.py:547-558` の `if gate_dtype.kind not in "iufb": ... continue` を外すと `Mix.tag` が Signal になり `test_non_numeric_leaf_is_not_a_column_key` の `assert loaded.resolve("Mix.tag") is None` が RED。実測: **1 failed / 10 passed**（`Mix.x` は巻き添えにならない = リーフ単位判定の確認）。

**(e) selector を表示キーから導く（二重名の selector 側・この fixture の最重要の歯）** — `sel_leaves = _flatten(base_name, samples)` → `_flatten(signal_name, samples)`。実測: **3 failed / 8 passed** — `test_every_numeric_column_key_resolves_with_matching_values` / `test_literal_values_pin_the_key_to_column_mapping` / `test_deduplicated_exploded_channel_pins_both_name_spaces`。エラーは `SampleReadError: 信号 'Mat2' のサンプル読み取りに失敗しました: 'Mat2[0][0]'`（遅延読みが生名で `_flatten` したリーフ集合に表示キーが無い）。

> **この歯は `Mat2` が無いと存在しない**: 同じ sabotage を Mat2 追加前のオラクル（e48e72a 版・10 本）に当てると **10 passed = 完全に盲目**だった（実測）。1-D の `dup` は selector について何も言わず、他の展開チャンネルは名前が一意で表示名 = 生名になるため、`display == base_name` が成り立ってしまい sabotage が観測できない。

**(f) comment を metadata から落とす** — `_extract_metadata` の `if asammdf_sig.comment: meta["comment"] = ...` を削ると `test_metadata_matches_oracle_for_every_column` が `assert meta.get("comment", "") == col.comment` （`AssertionError: Pos.xa`）で RED。実測: **1 failed / 10 passed**。fixture が `Scaled`（1-D）と `Pos`（展開親）に実 comment を持つので効く — 全チャンネルの comment が `""` だと両側とも空で通る vacuous な腕になる。
...
5. **Task 0 オラクルの sabotage（controller 指定・ここで初めて鋳造経路を通る）**:
   ...
   (c) **selector を表示キーから導く** — 鋳造した列の selector を `ColumnRecord.raw_base_name` ではなく表示キー由来にすると `test_deduplicated_exploded_channel_pins_both_name_spaces` ほかが RED（反転前の実測は 3 failed / 8 passed・`SampleReadError: ... 'Mat2[0][0]'`）。`Mat2` は生名を共有する 2 本なので、`parse_leaf` に**生名キーの `Mapping`** を渡す実装（docstring が自認する健全性の限界）もここで落ちる — 表示名キー + `ColumnRecord` 経由で生名を復元する設計であることを確認する。
   いずれも sabotage を戻して GREEN に復帰することを確認する。

   > **Task 0 の sabotage ロスターに関する注意**: T0 Step 5 の (b)（LD-08 dedup を殺す）で `test_oracle_covers_every_naming_arm` は **GREEN のまま**である（初版プランの予測は誤り・訂正済み）。このテストはオラクル側しか assert せず、オラクルは dedup を独立に計算するため production の dedup 破壊では動かない。予測との差分を「反転で何か壊れた」と読まないこと。
