...
- [ ] **Step 5: GREEN — realgui 実行＋スクショ目視**
...
`- [ ] uv run pytest --realgui tests/realgui/test_dock_onscreen_after_toggle.py の pass ログ＋スクショ2枚＋目視判定コメントを PR 説明/実行ログに残す（merge 前ゲート: (a) full pytest 0 fail ＋ (b) 本証拠 ＋ (c) CI 緑）`

- [ ] **Step 7: 品質ゲート（headless 全体の無回帰）**
