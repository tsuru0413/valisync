...
Task 1-7 完了（T1 `8741cd1`・T2 `1057136`・T3 `388da7d`・T4 `898fe79`+`8304564`・T5
`09e81eb`+`6c46736`・T6 `127b424`+`ee9c4d3`・T7 `f7f2f8b`）。①ゲート: headless 717
passed / 0 errors・realgui scoped 7/7。承認済み逸脱7件: 意味反転テスト（非単調
ValueError → 受け入れ assert への書き換え）／downsampler 非単調分岐（パススルー
を単調時のみ zero-copy に限定）／`_derive` 第4引数（`differentiate` も他3演算と
同様に整列済み `t` を渡す形へ統一）／namespaced 委譲キャッシュ（sorted_view を
元 Signal に委譲・オフセット適用ラッパーは対象外）／診断ジャンプ source 一本化
（signal_name 照合不能クラスの根絶＋FB spec §4.4 更新）／CSV 失敗 return の
`*diagnostics` 展開（後出し診断を落とさず集約済みリストを保持）／RUF 機械対応
（ruff の自動整形指摘への追従）。
...
- [x] **Step 4: Run tests / 既存 suite の影響確認**
...
- [x] **Step 3: Implement — mdf4_loader.py**
...
- [x] **Step 3: Implement — csv_loader.py**
...
- [x] **Step 2: Run test** → PASS（Task 6 まで完了していれば RED にならない想定 — 接続確認テストのため sabotage（csv_loader の warning 発行を一時的に外す）で「落ちること」を確認して復元。

- [x] **Step 3: `/gui-verify`（①ゲート）**

- [x] `git diff --name-only main...HEAD -- src/valisync/gui/` の変更が `graph_panel_vm.py`（ViewModel・入力経路なし）のみであることを確認 → **GUI 入力経路の変更なし＝realgui ゲート対象外**と判定を記録
- [x] 念のため描画系 realgui の回帰1本のみ scoped 実行: `uv run pytest --realgui tests/realgui/test_x_axis_zoom_pan.py`（render 供給変更の実機確認・マウス数秒占有）
- [x] headless full `uv run pytest` 0 errors
- [ ] CI 緑（push 後確認）
