...
- [x] **Step 1: WM リサイズプリミティブを共有ヘルパへ追加**
...
- [x] **Step 4: sabotage honest-RED — filter 設置を一時的に外して実 FAIL を実証**
...
- [x] **Step 6: GREEN — realgui 実行＋スクショ目視**
...
- [x] **Step 7: realgui レシピへ実 WM リサイズ節を追記**
...
`- [x] uv run pytest --realgui tests/realgui/test_busy_overlay_resize_realinput.py の pass ログ＋スクショ＋目視判定コメントを PR 説明/実行ログに残す（merge 前ゲート: (a) full pytest 0 fail ＋ (b) 本証拠 ＋ (c) CI 緑）`
