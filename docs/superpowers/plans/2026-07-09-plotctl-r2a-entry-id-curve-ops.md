...
**Architecture:** VM 側に `_PlottedEntry.entry_id`（単調増分）を導入し `RenderCurve` に載せる。View の内部辞書（`_items`/`_item_vb`）を entry_id キーへ移行し、同一 signal_key を複数 axis に載せても独立描画・独立操作できるようにする。**公開テストアクセサ（`curve_keys`/`curve_xy`/`pen_color`/`is_clipped`）は signal_key シグネチャを維持**（内部で first-match 解決）して既存 9 テストファイルの churn を回避し、重複対応の by-entry アクセサを新設する。曲線右クリックは `contextMenuEvent` のヒットテスト分岐（曲線→空白。軸分岐は増分2b・カーソル線分岐は増分3 で差し込む）で振り分ける。オフセットドラッグは DP16 に従い「press=候補保持→`startDragDistance` 閾値超えでドラッグ開始／閾値内 release=曲線活性化」へ変更する（grabMouse は press 時点で取得）。
...
- **entry_id の同一性**: `entry_id` は VM が採番する単調増分 int。同一 signal_key の複数エントリを一意に区別する。View の内部キーは entry_id、公開アクセサは signal_key シグネチャ維持（重複時は first-match）＋ by-entry アクセサを別途提供。
...
| 3 View 再キー化＋offset 再キー | ウィジェット構成・状態 | 一部（重複独立描画） | **必須**（既存 view/offset テスト無回帰＋重複描画） | task 8 で offset 実ドラッグ無回帰 |
...
- **曲線活性化の太線化**（task 4）: 「人間はクリック後に線が太くなるのを見て合格と判断」。自動アサート = `pen_width_by_entry(eid) == 2.5`（Layer B）。視覚確定 = 実クリック後のスクショ（Layer C）。**VM 状態の再チェックだけ・スクショ保存だけは naive** → 実際の pen 幅を測る。
...
- `_curve_at` の返り値を entry_id へ変える（task 3）が、これは内部専用（mousePressEvent/contextMenuEvent/`_hover_cursor` のみ参照）でテスト表面ではない。公開アクセサの signal_key シグネチャは維持するため、既存 view テストはハンドラを迂回せず無改変で通る（＝迂回近道ではない）。
...
| `src/valisync/gui/views/graph_panel_view.py` | Modify | 内部辞書の entry_id 化・`_item_signal_key`・`refresh` desired・`_curve_at→int`・オフセットドラッグ entry_id 化・公開/by-entry アクセサ・曲線活性化（`_active_curve_id`/`_activate_curve`/`_deactivate_curve`）・DP16 ジェスチャ・H キー・`contextMenuEvent` ルーティング・`build_curve_menu`・色ダイアログ DI |
...
**このタスクは atomic。** `_items` を entry_id キーへ移すと、signal_key で `_items` を引くオフセットドラッグが同時に壊れるため、両者を 1 コミットで移行する。曲線ドラッグの**タイミングは変えない**（DP16 は Task 4）。公開アクセサは signal_key シグネチャを維持し、既存 9 テストファイルを無改変で green に保つ。
...
- Produces:
  ...
  - 公開アクセサ（signal_key シグネチャ維持）: `curve_keys() -> list[str]`・`curve_xy(signal_key)`・`pen_color(signal_key)`・`is_clipped(signal_key)`
  - by-entry アクセサ（新設）: `entry_ids() -> list[int]`・`curve_xy_by_entry(entry_id)`・`pen_color_by_entry(entry_id)`・`pen_width_by_entry(entry_id) -> float`

- [ ] **Step 1: 失敗するテストを書く**（重複独立描画＋by-entry アクセサ）
...
```python
    def test_duplicate_signal_key_draws_independent_curves(
        ...
        assert len(view.entry_ids()) == 2  # type: ignore[attr-defined]
        # 公開アクセサ (signal_key) は後方互換: 同名 2 本ぶんの key を返す
        assert view.curve_keys() == [key, key]  # type: ignore[attr-defined]
        # by-entry アクセサでそれぞれ独立にデータを読める
        for eid in view.entry_ids():  # type: ignore[attr-defined]
            x, y = view.curve_xy_by_entry(eid)  # type: ignore[attr-defined]
            assert len(x) > 0
    ...
    def test_set_color_on_one_entry_repaints_only_it(
        ...
        vm.add_signal(key)
        vm.create_new_axis(key)
        e0, e1 = view.entry_ids()  # type: ignore[attr-defined]
        vm.set_color(e1, "#123456")
        view.refresh()  # VM notify 経由でも呼ばれるが、テストは明示的に
        assert view.pen_color_by_entry(e1).lower() == "#123456"  # type: ignore[attr-defined]
        assert view.pen_color_by_entry(e0).lower() != "#123456"  # type: ignore[attr-defined]
```

- [ ] **Step 2: 失敗を確認**

Run: `uv run pytest tests/gui/test_graph_panel_view.py -k "duplicate_signal_key or set_color_on_one_entry" -v`
Expected: FAIL（`entry_ids`/`curve_xy_by_entry`/`pen_color_by_entry` 未定義・現状 `{c.name: c}` で同名衝突し 1 本に collapse）
...
- [ ] **Step 5: 公開アクセサを signal_key 解決に、by-entry アクセサを新設**

公開アクセサ（:1179-1193）を差し替え:

```python
    def _entry_for_signal_key(self, signal_key: str) -> int:
        """Resolve the first drawn entry_id with *signal_key* (legacy accessors).

        Existing tests address curves by signal_key and never create duplicate
        keys, so first-match is exact for them.  Per-entry operations and the
        duplicate-aware tests use the entry_id accessors below.
        """
    ...
    def curve_keys(self) -> list[str]:
        """Return the signal keys currently drawn, in draw order (may repeat)."""
        return [self._item_signal_key[eid] for eid in self._items]

    def curve_xy(self, key: str) -> tuple[object, object]:
        """Return the (x, y) arrays of *key*'s curve (signal_key, first match)."""
        return self._items[self._entry_for_signal_key(key)].getData()

    def pen_color(self, key: str) -> str:
        """Return the hex colour of *key*'s curve pen (signal_key, first match)."""
        eid = self._entry_for_signal_key(key)
        return pg.mkPen(self._items[eid].opts["pen"]).color().name()

    def is_clipped(self, key: str) -> bool:
        """Return whether *key*'s curve is clipped (signal_key, first match)."""
        eid = self._entry_for_signal_key(key)
        return bool(self._items[eid].opts.get("clipToView", False))

    # ── entry_id-addressed accessors (duplicate-aware; per-entry operations) ──
    def entry_ids(self) -> list[int]:
        """Return the entry_ids currently drawn, in draw order."""
        return list(self._items)

    def curve_xy_by_entry(self, entry_id: int) -> tuple[object, object]:
        """Return the (x, y) arrays of the curve for *entry_id*."""
        return self._items[entry_id].getData()

    def pen_color_by_entry(self, entry_id: int) -> str:
        """Return the hex colour of the curve pen for *entry_id*."""
        return pg.mkPen(self._items[entry_id].opts["pen"]).color().name()

    def pen_width_by_entry(self, entry_id: int) -> float:
        """Return the pen width of the curve for *entry_id* (active curve = 2.5)."""
...
- [ ] **Step 8: 新規テスト pass ＋既存 view/offset/dnd テスト無回帰**
...
Expected: PASS（新規2本＋既存すべて。公開アクセサの signal_key 互換で既存は無改変）

- [ ] **Step 9: mypy（辞書型の変更に追随）＋コミット**
...
git commit -m "refactor(gui): GraphPanelView 内部辞書を entry_id キーへ移行 (重複独立描画・PC-01)"
```
...
**Interfaces:**
- Consumes: Task 3 の `_curve_at -> int`・`entry_ids`・`pen_width_by_entry`・`GraphPanelVM.axis_of_entry`
- Produces:
...
```python
    def test_click_on_curve_activates_it_thick_pen(
        ...
        view.refresh()
        eid = view.entry_ids()[0]  # type: ignore[attr-defined]
        ...
        assert view.pen_width_by_entry(eid) == 2.5  # type: ignore[attr-defined]
    ...
    def test_click_within_threshold_does_not_offset(
        ...
        ...  # 上と同じセットアップ
        before = np.asarray(view.curve_xy_by_entry(eid)[0]).copy()  # type: ignore[attr-defined]
        view.mousePressEvent(_press(pos))
        view.mouseReleaseEvent(_release(pos))
        after = np.asarray(view.curve_xy_by_entry(eid)[0])  # type: ignore[attr-defined]
...
`test_curve_direct_ops.py` に、実起動→信号を1本プロット→曲線上を実クリック→活性化（太線）のスクショ→実 H キーで非表示→再表示、を閉ループで検証。曲線ヒット座標は `curve_xy_by_entry`＋`plot_widget.mapFromScene` で算出（memory `gui_realgui_zone_widgetspace_and_offscreen_clamp` の widget 空間座標算出）。活性化の視覚確定はスクショ AI 判定、状態確定は `view.active_curve_id()`＋`pen_width_by_entry`。
...
**3. 型整合:** `entry_id: int` を VM（`_PlottedEntry`/`RenderCurve`）→View（`_items: dict[int,...]`/`_curve_at -> int | None`/`_offset_drag_key: int | None`）で一貫。公開アクセサは `str`（signal_key）シグネチャ維持、by-entry アクセサは `int`。`signal_key_for_entry -> str | None`・`axis_of_entry -> int | None`・`color_dialog_fn: Callable[[], str | None]` が各消費側と一致。
