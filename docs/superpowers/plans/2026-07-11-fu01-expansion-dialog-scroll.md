...
- [ ] **Step 5: sabotage honest-RED — クランプを一時的に外して実 FAIL を実証**
...
- [ ] **Step 7: GREEN — realgui 実行＋スクショ目視**
...
`- [ ] uv run pytest --realgui tests/realgui/test_expansion_dialog_realinput.py の pass ログ＋スクショ＋目視判定コメントを PR 説明/実行ログに残す（merge 前ゲート: (a) full pytest 0 fail ＋ (b) 本証拠 ＋ (c) CI 緑）`
