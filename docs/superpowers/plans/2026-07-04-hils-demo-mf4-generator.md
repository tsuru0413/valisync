...
## Status: 実装完了（dde7c00..83cc1f6・最終パネル ship）

全5タスク消化。各タスクで3レンズ並列レビュー→fix→GREEN、最終判定パネル（completeness/integration/triage）全レンズ ship。742 passed/40 skipped・ruff/mypy green。hils 実測 2.01GB/42.3s・quick 167MB/4.3s（調整不要）。

**レビュー駆動でプラン当初スニペットから逸脱した点**（以下はプラン本文の code snippet より優先・詳細は各 task report / ledger）:
- T2 Finding: 2D は structured-dtype ではなく **uint8 非構造化 byte-array**（structured は Mdf4Loader で ndim==1 に見え skip されず偽データ化）。プラン step-3 の記述より本節が正。
...
- [x] **Step 3: Implement — scripts/generate_demo_mf4.py（骨格＋シナリオ）**
...
- [x] **Step 2: RED 確認** → `uv run pytest tests/test_demo_mf4.py -k smoke -v` FAIL（write_mf4 未定義）。

- [x] **Step 3: Implement — グループ定義と書き出し**
...
- [x] **Step 4: Run tests / gates** → smoke roundtrip＋再現性 PASS・全体ゲート緑。
...
- [x] **Step 4: Run tests / gates** → PASS・全緑。
...
- [x] **Step 2: docs/development.md に使い方節**
...
- [x] **Step 1:** `uv run python scripts/generate_demo_mf4.py --profile hils --out demo_data/hils_demo.mf4` を実行（分オーダー・進捗表示）。
- [x] **Step 2:** 実測サイズが 1.6-2.4 GB を外れる場合は `Ctrl.Internal[NN]` の本数（XCP_1ms の ch 数）を調整して再生成（コード定数の変更＋テスト影響確認）。
- [x] **Step 3:** 実測結果（サイズ・生成時間・ピークメモリ概算）を docs/development.md の使い方節に追記。quick も1回生成して同様に記録。
- [x] **Step 4:** 全体ゲート→Commit（調整があった場合）。
