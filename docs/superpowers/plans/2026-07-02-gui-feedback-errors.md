...
- [ ] **Step 1: Write the failing test（成功時に診断を返す・LoadError が診断を持つ）**
...
- [ ] **Step 3: Implement — `session.py` を変更**
...
- [ ] **Step 4: Migrate `request_load`（production）**
...
- [ ] **Step 5: Migrate 直接 `session.load` の戻り値使用箇所（テスト）**
...
- [ ] **Step 1: Write the failing test（finished が LoadOutcome、failed が例外を運ぶ）**
...
- [ ] **Step 3: Implement — `load_worker.py` を変更**
...
- [ ] **Step 4: 既存 LoadController テストを追従**
...
- [ ] **Step 1: Write the failing test（Layer B: 描画・フィルタ・Clear）**
...
- [ ] **Step 1: Write the failing tests（Layer A 配線）**
...
- [ ] **Step 1:** `/gui-test-plan` を実行し、`DiagnosticsView` の入力経路（フィルタボタン clicked、行ダブルクリック→`entry_activated`→アクティブファイル切替）について②実質的な受け入れ要件（Layer 判定・realgui 要否）を設計する。ダブルクリック→ジャンプは Layer C 候補。
- [ ] **Step 2:** 設計に従い Layer B（合成 double-click → signal emit）を `tests/gui/test_diagnostics_view.py` に追加。必要なら Layer C（`tests/realgui/`）を追加。
- [ ] **Step 3:** merge 前に `/gui-verify` を実行し、①realgui 証拠ゲートを満たす（realgui が skipped=検証済みと誤認されない）。
- [ ] **Step 4: Commit**（テスト追加分）。
