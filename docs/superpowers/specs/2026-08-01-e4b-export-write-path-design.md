...
- 点 (a) 読み時間 = 4,004 × 0.0055474 s + 320 × 0.16061 s ≈ **73.61 s**
  （丸め前の実測平均を使用。定数として保持する丸め値 0.00555/0.161 で
  計算すると 73.74 s になる — Task 11 再レビュー N3: 較正には丸め前の値を
  使ったので、電卓で再現したい場合はこの丸め前の平均を使うこと。旧仮定の
  1,366.4 s からは 1/18.6 に縮小）。
...
**適用域注記（Task 11 再レビュー N1・C1 と同型の縮小版）**: wide 単価の母集団は
prod_demo の leaf_count 分布に限られ、内訳は `{1000: 260 本, 1100: 60 本}` —
**2〜999 leaves のチャンネルは母集団に 1 本も無い**。コストは幅にほぼ比例せず
（leaf_count=1 → 1,019 で 28.9 倍・幅は 1,019 倍）固定費支配のため、細い wide
にこの単価を適用すると過大請求になる: quick_demo の 8-leaf チャンネルは
cold select 実測 **1.293 ms** だが、本単価は **161 ms** を課金する（~124 倍
過大・別ファイル間比較のため絶対値の厳密比較ではないが方向は明白）。
過大方向の誤りは確認閾値に対しては安全側（見積が実態より重く出て確認を
余分に出す方向）だが、絶対時間の見積としては信頼できない。現行データセット
への実害はゼロ（prod_demo に該当なし・quick_demo で 2 本 ≒ 0.3 s）。恒久対応は
leaf_count（またはバイト数）による内挿だが、本増分の範囲外（未着手・merge
ブロッカーではない）。
