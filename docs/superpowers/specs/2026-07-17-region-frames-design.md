...
separator と 1px 枠は同一役割なので単一トークンを共有する。**両値は既存 `border_chip`
（チップ枠）と同値** — 役割が違うため別トークン（docs/design.md 原則1）。同値別トークン
の盲点（ピクセル比較・値ベース assert が誤配線に盲目 — memory
gui_freeze_tokenization_verification_pattern）は、chrome_frame を分岐させたテーマで
qss 生成関数の消費トークンを直接 assert する値分岐テストで閉じる。
golden スナップショット（DARK/LIGHT 全域 test-lock）を更新する。
