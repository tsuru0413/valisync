...
### §1.5 ツリーを縮小可能にして最小ドック幅を実際に下げる（Task 4 実測で追加確定）
**Task 4 の実測で、最小ドック幅の律速はヘッダー/タイトルバーでなく「ツリー本体の sizeHint（256px・名前列＋単位列）」と判明**
（実 MainWindow を 3 条件で構築し `channel_dock.width()` が全条件 258px）。ヘッダーのファイル名廃止（§1 変更）だけでは最小幅は
下がらないため、**ツリーの列を縮小可能にして最小幅を下げる**:
- ツリー（`ChannelBrowserView.tree`）の列（名前・単位）を**縮小可能**にする（`QHeaderView` の `minimumSectionSize` 縮小／
  名前列の `resizeMode` を Interactive 等にして詰められるように・長い名前は elide「…」で省略）。ツリー widget の
  `minimumWidth`/`sizeHint` を下げ、ドックを 258px より細くドラッグできるようにする。
- **目標下限 = タイトルバー律速（~181px・ユーザー承認済み「約半分で十分」）**。ツリーを縮小可能にすると次の律速は
  `CollapsibleDockTitleBar`（chevron＋タイトル＋float/✕・推定 ~181px）へ移る。**181px より下（タイトルバーの文字省略・
  ボタン削減）は本増分スコープ外**（ユーザー決定「181 で十分」）。
- **実測で下限を確定**（Task 4 の教訓＝机上見積りは外れる）: 実装後に実 window でツリーを最小まで詰めた `channel_dock` の
  実幅を測り、258px より有意に小さい（目標 ~181px 近傍）ことを実証・実際の数値を報告。推定 181px と乖離すれば原因特定。
- **トレードオフ**: 細くすると長い信号名（`Radar.Obj0.dx` 等）は「…」省略。列ヘッダのドラッグで再拡大可・
  PC-19 リッチツールチップで全名確認可（既存）。単位列は詰めるか据え置き。
- 既存のソート/フィルタ/D&D・選択（`mapToSource` 系）は列幅変更で無回帰であること。
