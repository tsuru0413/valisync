...
## 増分③ VM-side sort 実測（2026-07-14・prod 264k）

proxy 撤去（増分①）で失った PC-20 列ソートを `SignalTreeModel` 自身へ移した VM-side sort を prod で実測:
- **遅延保持を実証**: `model.sort` は `_top`（4,264 base）と**既に materialize 済**の children のみ並べ、未展開の親は触らない。load 後 baseline / 全 sort 後とも **materialized=0**（proxy が誘発した 260k eager materialization を再導入しない）。1 親を展開してから sort しても materialized は 1→1（拡散しない）。
- **非フリーズ**: `model.sort(0,Asc)`=90ms（top0='AEB.TTC'）・`sort(0,Desc)`=93ms（top0='VehSpd'）・`sort(1,Asc)`unit=96ms・実ヘッダ経路 `tree.sortByColumn(0,Asc)`=80ms。いずれもフリーズ閾値以下。
- 大小無視（`.lower()`・col0=Name/col1=Unit）で混在ケース ADAS 名を A-Z 連続化。`node.row` を並べ替え後に全再割当（`parent()`/`index()` が row 依存）。`_rebuild` で active sort を再適用しフィルタ/信号変更を跨いで維持。
- **view 側の落とし穴（TDD 捕捉）**: `setSortingEnabled(True)` は Qt の古い既定ヘッダインジケータ（col 0 Desc）を本物 sort として発火させ `sort(-1)` passthrough では復元不能 → `header().setSortIndicator(-1, ...)` を enable 前に置く（sortable だが起動時 unsorted の Qt 定石）。
- **latent（unreachable・cleanup wave）**: `sort(-1)` は real sort 後の `_top` を session 順へ復元しない（現 codebase に到達経路なし＝clear-sort UI/3-state header 無し）。**増分③ 完了。**
