...
- `split_key(signal_key) -> tuple[str, str]` — `(group_key, bare_name)`（`KEY_SEPARATOR` は signal_group_manager から import。**channel_browser_vm.py:19 のローカル `_SEP` 重複定義もこれへ統一**）。**セパレータ無し入力の契約: `("", 入力)` を返す**（全文字列を名前側へ — channel_browser_vm.py:113/graph_panel_vm.py:479 の既存フォールバック慣行と同方向・bare が空になる縮退を作らない）。
...
- **`csv_header_names(keys) -> dict[signal_key, str]`（第 4 の API — CSV ヘッダ専用の集合スコープ解決）**: 衝突判定は display_names と同じ distinct group_key 規則だが、**母集合に固定列ヘッダ `"timestamp"` を裸名として常時注入**し（signal_key でない値を扱うのはこの API 内部のみ — split_key の契約に依存しない）、衝突キーへは下記 `csv_header_name` 形式を返す。§1.2 のエクスポート行はこの API を使う（display_names〔表示形式・空白含み〕を CSV に使ってはならない）。
- **CSV ヘッダ専用形式 `csv_header_name`**: `"{bare}({group_key})"`（**空白なし** — エクスポートは区切り文字「スペース」を正式提供し exporter は無クォート join のため、空白含み形式はヘッダ列構造を破壊する〔レビュー実測: pandas が無言列ずれ〕。表示 UI の「{bare} ({group_key})」とは別形式であることを設計判断として記録）。
...
| 信号プレビュー「名前」行 | namespaced（signal_preview_vm.py:49） | windowTitle と同一規則（`display_names`・衝突スコープ=全ロード信号 — 同一ウィンドウ内でタイトルと名前行の識別情報が食い違わない） |
...
- 挙動（MainWindow ハンドラ — `_add_to_active_panel` と同じ配送先解決）:
  ...
  5. **[idx] 曖昧化の既知の限界（レビュー実測）**: dedupe サフィックス付き裸名は基準/対象で付与の非対称・走査順対応になり得るため**自動照合から除外し「曖昧」として別計上**（誤ペア重ねは全ゲートで検出不能な静かな事故 — 安全側。design.md へ設計判断として記録）。**判定機構（クローズ検証捕捉 — 名前文字列からは判定不能）**: LD-14 の配列展開名 `Name[i][j]` は決定的・ファイル間照合可能で除外**しない**が、LD-08 dedupe の `[idx]` と文字列上同型（中間形 `M[0][i]` も存在 — mdf_loader.py:451-452）。**ローダーが dedupe サフィックス付与時（mdf_loader.py:416-419・csv_loader.py:102-117）に `metadata["name_deduplicated"]=True` を記録し、照合除外はこのフラグのみを根拠とする**（core 変更 1 点・表示には使わない）
- 結果要約: `set_status_message(...)` — `{表示名}` は source_name・**重複 basename 時はキー併記（判断点 4 と同規則）**。計数は**母数=基準エントリ**で「n 件を重ねました（同名なし m・単位不一致 k・済み j・曖昧 i — 0 の項は省略）」・全件済みは専用文言「すべて重ね済みです」・**母数 0（アクティブパネルに基準由来エントリなし）は専用文言「基準の信号がプロットされていません」**（いずれも strings.py 定義）。timeout_ms=8000（既存 API — main_window.py:754-769）。**診断への記録はしない**（GUI 発診断の新機構は YAGNI — 初版モックからの意図的簡略化として記録）。
...
- **割当規則**:
  ...
  - **`variant_step` はモードに関わらず全 add で確定する**（1 ファイル時は色に未使用だが記録 — 1→2 ファイル遷移の reapply が「step 保持」でも家族内バリアントが最初から分離している。**クローズ検証 Critical の根治**）。確定規則: 同一パネル内の同一ファイル由来エントリ（**color_is_auto を問わず数える** — ピン留め前の自動色との重複を避ける安全側）が現在使用中の段を避けて**最小の空き段（0/1/2）**（削除の歯抜けを再利用）・全段使用中は **「当該ファイルの使用中 step 総数 mod 3」で決定的に**選ぶ。
  - **resolver が int（比較モード）**: `base = palette[hue]`・色 = `hue_variant(base, variant_step)`。**reapply は hue のみ再解決し step は保持** — 後続ロードで無操作の曲線が変色しない（「色の安定性優先」と整合・レビュー Critical の根治）。クロスパネル移送（extract/insert_axis）は entry ごと color/step を保持（既存挙動）。
...
- **Layer A 追加（レビュー反映）**: 同一キー重複プロット時は裸名のまま（衝突誤発火なし）・reapply の冪等性＋「削除歯抜け後の add が空き段再利用」＋「移送後のロードで step 不変」・**「1 ファイル時に 3 本 add→2 ファイル目ロード→reapply 後の 3 本が相異なる variant」（遷移分離 — クローズ検証 Critical の検出網）**・「2→1 ファイル遷移後の新規追加が count-mod へ戻る」・reapply 後の `render_data()` が新色を返す（VM 状態でなく curve を assert — cache 無効化の検証）
...
- **凍結検証（per-state 契約 — レビューが実ベースライン読取で確定）**: **完全一致 = 01/07/08**（08 のプレビュー撮影は「プレビュー」タブで名前行は写らない — 初版記述を訂正）／**意図差分 = 02-05/09**（読み値ペイン領域の名前のみ）／**06 = ツリー葉テキスト＋ダイアログ幅のサイズ変化許容** — サイズ不一致時 compare は当該状態を pixel 照合せず run 全体 exit 2 を返す（compare_screenshots.py:97-99/:126 — 初版引用 :73-75 は crop-meta スキップ経路で誤り）。**exit 2 の許容は「NG/サイズ不一致行が 06 のみの場合」に限定**し、06 は**代替検証（リサイズ前後の目視記録 — 葉テキストの E-0 変化と他退行なしの確認）**を必須とする（無検証通過を作らない — §9 と整合）。プロット viewport crop は全状態一致・色不変。バッジ/チップは比較モード限定のためカタログ不変（§2）。
