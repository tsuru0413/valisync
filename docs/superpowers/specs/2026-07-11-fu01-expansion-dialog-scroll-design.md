...
## 実装ノート（Task 2 実測で判明・2026-07-11）

Qt の `QScrollArea.sizeHint()` は `AdjustToContents` でも約 36×24 × フォント高で bound される（幅・高さともフォント高の倍数）ため、通常のデスクトップ画面では**スクロール化だけで**ダイアログは画面内に収まり、高さクランプは発火しない。クランプは bound（フォント/スケール依存）が画面高を超える環境（大フォント・低解像度）への**防御層**として保持し、ロジックは純関数 `_clamped_size` に抽出して直接テストする。受け入れ基準・要件に変更なし。
