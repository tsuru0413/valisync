...
**二層構造**（Task 1 レビュー Important 是正 — データセルにティントを敷きつつ `setForeground` なしで
文字色を既定 `chrome_text` 固定にすると、ティントを敷いたセル内のデータ数値自体が両テーマで不可読になる
[全4ケース 1.11〜1.96:1] ことが判明したため、**データ可読性**と**列識別（マーキング）**を別セルの責務に分離する）:

- **データセル契約（ハード制約）**: セル背景の低 α ティントは、データ文字（既定 `chrome_text`・`setForeground`
  しない）とのコントラストが**両テーマで AA（≥4.5:1）を保つ**こと。α を上げるほど実効セル背景が濃色化し
  `chrome_text` が埋没するため、α は可読性の上限で決める（非テキスト 3:1 の到達可否は問わない — 列識別は
  ヘッダ側が担う）。
- **列マーキング契約**: 列識別（どの列が時間/信号列か）は**ヘッダセル**が担う。ヘッダセルの背景は不透明
  （またはほぼ不透明）の強い色（`chrome_cursor_a`=時間列 / 信号ハイライトトークン=信号列）、文字色は
  背景輝度に基づき黒/白等から選んだ高コントラスト色（実効ヘッダ背景に対し **AA ≥4.5:1** — これは非テキスト
  3:1 の上位互換なので契約を満たす）。
- 両契約とも**値ベース機械検証で test-lock**（データセル: データ文字 vs 実効セル背景 ≥4.5:1・両テーマ／
  ヘッダセル: ヘッダ文字 vs 実効ヘッダ背景 ≥4.5:1・両テーマ）。sabotage（データセル α 過大・ヘッダマーキング
  欠落や閾値未達）が両テストを独立に RED にすることを実証する。
