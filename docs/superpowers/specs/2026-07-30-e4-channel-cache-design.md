...
5.8 ms が構造的な床」を実測した先例に倣う）。

（実測は**追い出される配列の出自ごとに 2 つ記録する** — T5 レビュー I1。

| 何を追い出したか | n | 中央値 | 最大 |
|---|---|---|---|
| **実 asammdf デコード配列**（prod_demo 広幅 12.6 MiB/本・キャッシュが唯一の所有者を refcount で確認） | 20 × 10 run | **1.2–1.7 ms** | 1.5–4.2 ms |
| **合成 `np.ones` 13.2 MB を空キャッシュへ積んで追い出す**（T4 が採った方法論） | 20 × 6 run | **0.65–0.83 ms** | 0.7–3.3 ms |

T4 が記録した「最大 1.28 ms / 中央値 0.65–0.79 ms」は**後者だけ**の値である。~2 倍の差は
手法の差ではなく**主語の差** — 合成側は free した 12 MB ブロックを次のループが即座に
再確保する **warm-allocator バイアス**が乗る。分離は同一プロセスで示せる: 飽和した実
キャッシュへ同じループで put し続けると、実配列 20 本を出し切った瞬間に中央値が
1.4 ms → 0.67 ms へ落ちる（そこから先はリサイクルされた `np.ones` を落としている）。
サイズは効かない（合成 11.99 MiB と 12.59 MiB は同値）。

**ページを dirty 化して計測**すること: `np.zeros` の未タッチページは lazily-zeroed で
free が `VirtualFree` の実体を伴わず、同条件で最大 0.045–0.051 ms＝**~17–25 倍の過小**。
`np.ones` はこれを回避するが、**合成であること自体のバイアスは回避しない** — 実測を
記録するときは「何がそのバイトを作ったか」を必ず併記する。）

**この数値は teardown のペーシングの床を上げない**: 床は「デッドライン + 最大単発解放」
（memory `perf_pacing_floor_is_deadline_plus_largest_atomic_unit`）で、E-3 T9 が実測した
最大単発 Signal 解放 5.8 ms のほうが依然として支配的である（実配列の中央値 1.2–1.7 ms
でも変わらない）。
...
| キャッシュ飽和の会計（**一次**） | — | **`bytes_held ≤ 256 MB`**（実測 251.8 MB・evictions ≥ 1・`skipped_oversize` 0） |
| キャッシュ飽和後の USS（**参考**） | — | **ベース +（256 + 実測オーバーヘッド）以内**。~~ベース +256 MB 以内~~ は**達成不能な帯だった**（T5 レビュー I4）: LRU は「次の 1 本が入らない」ところまで正当に保持する（251.8 MB）ので USS 側のヘッドルームは構造的に ~4 MB しか残らず、アロケータ/一時分（実測 headless 2.3–2.8 MB・実 GUI 4.7 MB）がそれを食う。判定は `bytes_held` を一次にし、USS は 256 + 8 MB の参考帯で読む |
| LRU evict 1 回の同期解放時間 | — | **主語ごとに 2 つ記録する**（T5 レビュー I1）。**実 asammdf デコード配列**（12.6 MiB/本・キャッシュが唯一の所有者）= 中央値 **1.2–1.7 ms** / 最大 1.5–4.2 ms（n=20 × 10 run）。**合成 `np.ones` 13.2 MB を空キャッシュへ積んで追い出す**（旧記録の方法論）= 中央値 **0.65–0.83 ms** / 最大 0.7–3.3 ms（n=20 × 6 run）。~~最大 1.28 ms / 中央値 0.65–0.79 ms~~ は後者だけの値で、**warm-allocator バイアス**（free した 12 MB ブロックを次のループが即再確保）で ~2 倍楽観。サイズは効かない（合成 11.99 MiB と 12.59 MiB は同値）。**ページを dirty 化して計測**すること（`np.zeros` の未タッチページは ~17–25 倍の過小）。どちらの主語でも E-3 T9 の最大単発解放 5.8 ms 未満なのでペーシングの床は上がらない |

`--core` モード（E-3 T9 新設）と実 GUI の両方で測る。**メモリの主指標は USS**（RSS は
mmap 済み共有ページを含む）。ただし**キャッシュ飽和の判定だけは `bytes_held` が一次**で、
USS は参考である（上表 I4）。

**「1,584 ms → ≤ 380 ms」の読み方**（T5 実測・レビュー I2/I5）:

- **現在（真の before）**は位置で 2 通り測れている。E-3 が計測器の定位置（GUI・先行段で
  ファイルが温まっている）で測ったのが **1,584 ms**、merge-base 84a5a69 を実際に
  チェックアウトして load 直後に測ると **2,033 / 2,102 / 2,177 ms**（n=3・同一
  チャンネル `Prod10Wide_0000`）。計測器が同一実行内に出す対照（列ごとに 1 decode）は
  **1,675–1,809 ms** で、merge-base より中央値 ~380 ms 安い — 対照は「以前の経路」では
  なく**その下限**である（対照は `select` だけを払い、merge-base は列ごとに全リーフの
  `_flatten` もやり直していた）。
- **達成後**の 5 列合計は実質「1 チャンネル decode 1 回」であり、**その 1 回の分散が
  そのまま帯の裁定になる**。HEAD 実測 n=28（fresh process・計測器の定位置 16 / load 直後
  12）で中央値 **345 ms**・**4 件（14%）が 380 ms を超過**・最大 513 ms。超過は
  **両方の位置で**起きる（定位置 1/16・load 直後 3/12）。兄弟列は全 28 実行で
  0.07–0.31 ms・`select` は常に 1 回なので、**残差は E-4a が触っていない asammdf の
  decode + I/O** である（E-4b の対象）。帯は「1 回の decode が入るか」を見る
  **advisory** として読むこと。
