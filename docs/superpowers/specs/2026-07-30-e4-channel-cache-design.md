...
（T4 実測: **最大 1.28 ms / 中央値 0.65–0.79 ms**・1 エントリ 13.2 MB・20 回 × 3 run・
**ページを dirty 化して計測**。`np.zeros` の未タッチページは lazily-zeroed で free が
`VirtualFree` の実体を伴わず、同条件で最大 0.045–0.051 ms＝**~17–25 倍の過小**になる。
実キャッシュエントリは asammdf の decode 結果なので全ページ dirty であり、後者が実態。）
...
最大単発 Signal 解放 5.8 ms のほうが依然として支配的である（1.28 ms < 5.8 ms）。
...
| LRU evict 1 回の同期解放時間 | — | **実測 1.28 ms**（最大・20 回 × 3 run・1 エントリ 13.2 MB・**ページを dirty 化して計測**: `np.zeros` の未タッチページでは ~17–25 倍の過小になる。E-3 T9 の最大単発解放 5.8 ms 未満なのでペーシングの床は上がらない） |
