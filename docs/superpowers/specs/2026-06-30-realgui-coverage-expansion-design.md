...
- **ChannelBrowser / DataExplorer**: コンテナ override を撤去し、子 QTreeView に `setContextMenuPolicy(Qt.CustomContextMenu)` ＋ `customContextMenuRequested` シグナル→`indexAt(pos)` でカーソル下行→メニュー構築（FileBrowser 修正と同型）。
