# valisync-gui-axes R1（複数列 Y軸グリッド）follow-up

> 関連 spec: `valisync-gui-axes`（「完了」扱い・tasks 全 `[x]`・PR #4 merged）。
> 発見の経緯: 「multi-axis の要件に Y軸の複数列表示はあるか」という確認から、**R1（Multi-Column Y-Axis Grid）が spec/tasks 上は完了扱いだが、実コードのレンダリングでは未実装**であることが判明した（当時）。
> **→ 2026-06-27 に `feature/valisync-gui-axes-multicolumn` で実装完了（下記参照）。**

## 実装完了 (2026-06-27)

R1 Multi-Column Y-Axis Grid は `feature/valisync-gui-axes-multicolumn` ブランチで実装完了（PR pending）。実装計画 `docs/superpowers/plans/2026-06-27-multi-column-y-axis.md` に沿った内容:

- **レイアウト**: `_reconcile_axes` が列ごとに `GraphicsLayout` を生成 (`_axis_layouts: dict[col → GraphicsLayout]`); プロット ViewBox は root col = `column_count`; 空き列は固定幅スロット ("余白") として残りドロップターゲットになる。
- **ドロップルール (R5 改訂)**: 軸上へのドロップ = `overwrite_axis`（上書き）; **Ctrl+ドロップ = `add_signal_to_axis`（追加）**; 背景ドロップ = `create_new_axis`。
- **Rule A (新規軸配置)**: 新規軸は内側列 (`column_count−1`) の末尾に追加（`create_new_axis`）。
- **軸移動 D&D + ドロップフィードバック（案1）**: 挿入線（occupied 列）・列全体ハイライト（空き列）・移動元淡色プレースホルダ。
- **既知の小挙動**: `column_count` を増やしても既存軸は自動で外側列へ移動しない（列割当はそのまま）。手動で軸 D&D により再配置できる。
...
> 保留項目・実装完了ステータスについては冒頭の「**実装完了 (2026-06-27)**」セクションを参照。
