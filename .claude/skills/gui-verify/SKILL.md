...
- **(d) CI 緑**（push 済みなら確認）。
- **(e) ジャーニースモーク（無条件・diff スコープ判断の外）**: GUI 変更の gate では `tests/realgui/test_journey_smoke.py`（実 `MainWindow`＋実 OS 入力で基本ジャーニー一気通貫: 開く→プロット→軸クリック活性化→**続くジェスチャがユーザー可視の効果を生む**→カーソル→閉じる）を毎回実行。相互作用バグ（A の変更が B を壊す）は scoped 選定が構造的に取りこぼす — 機構盲目の無条件チェックで受ける（FU-15: 個別テスト全 pass のままジャーニーだけが壊れた）。スモーク未新設の間は、変更領域の基本ジャーニーを実 `MainWindow`＋実 OS 入力の診断スクリプトで代替実測し、新設を課題化する。
...
- **介入コンポーネント/祖先チェーン不在のハーネスで pass する realgui** → 実経路 exercise 欠（app フィルタ等のグローバル介入は実組立てでのみ存在・未 accept press の祖先バブルは top-level 化ハーネスで消える。FU-15）。
...
4. **先回り E2E を実行**:
   ...
   - **クロスカット変更**（`QApplication`/共有祖先への `installEventFilter`・グローバルショートカット・app 全体のイベント/フォーカス監視）: 名前マッピングでは影響圏が出ない（介入は**全ウィジェットの入力**に作用）→ 介入コンテナ内の**入力経路 realgui クラスタを全実行**し、さらに**介入コンポーネント＋祖先チェーンが実在する組立てハーネス（実 `MainWindow`）で「既存ジェスチャが完遂する」証拠を最低1本**（無ければ追加してから gate）。**「ジェスチャ完遂」= 活性化に続く操作がユーザー可視の効果を生むまで**（例: 実クリック活性化→実 grip ドラッグ→高さ/レンジが実際に変化）。「活性化状態が戻る」だけでは完遂でない — FU-15 実バグでは活性化クリックは release 後勝ちで生き残り、続くドラッグ系だけが全滅した。**「bare ハーネスのテストはフィルタ未設置だから非曝露（zero exposure）」は除外理由にならない** — それは「そのテストが干渉を検出できない」という意味であり「その挙動が安全」という意味ではない。むしろその挙動こそ組立てハーネスでの再検証対象（FU-15: bare パネルの軸ジェスチャ realgui は全 pass のまま実アプリでジェスチャ全滅）。
