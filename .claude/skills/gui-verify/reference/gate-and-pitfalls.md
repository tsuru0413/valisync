...
2. **該当 realgui をマッピング**: `tests/realgui/test_*.py` を全列挙し、各本文が変更ファイルのモジュール/ウィジェット/関数名を参照するかで対応付け（`grep -l <識別子>`・固定表に頼らない・1変更が複数に対応し得る）。対応 realgui が無い経路はフラグ（黙って pass しない・`realgui-recipe.md` で追加するか観測のみで足る理由を明記）。
