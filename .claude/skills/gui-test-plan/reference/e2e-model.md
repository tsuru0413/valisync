...
| **グローバル介入**（`QApplication`/共有祖先への event filter・グローバルショートカット・app 全体監視） | **必須** | 発火側のみ可 | **必須**（実 `MainWindow` 組立てで**発火＋非発火〔既存ジェスチャ完遂〕の両方向**） | — | — |
...
### app レベル event filter は合成 notify・部分組立てで検証できない（FU-15 退行・2026-07-13 実測）
実クリック1回で `MouseButtonPress` は**複数配送**される: `QWidgetWindow`(QWindow)→target widget→（誰も accept しなければ）**親へバブルし介入コンテナの祖先（central stack/MainWindow）へも届く**。`QApplication` フィルタは**その全配送**を観測するため、「対象が subtree 外なら解除」型の条件は**祖先バブル配送で誤発火**し得る（FU-15: 軸上の press が自らアクティブ軸を解除→ジェスチャ全滅。純クリック活性化だけ release の `mouseClickEvent` 後勝ちで生き残り「解除しかできない」に見える）。合成 `notify(target, ev)` は1配送のみ・bare コンポーネントハーネスはフィルタ未設置・介入コンテナを top-level 化すると祖先が消えバブルが止まる — **いずれも構造的に見逃す**。非発火側の検証は**実 `MainWindow` 組立て＋実 OS 入力**のみ。
