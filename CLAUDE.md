...
| 横断 / デザイントークン・パイプライン | `gui/theme/` 意味名トークン基盤（単一の真実）＋Claude Design（claude.ai/design）連携の継続デザイン運用ループ | **増分1（凍結トークン化）完了（PR #112 merged）** — `theme/tokens.py`（pure Python・`Color` 正規化型・`active()` 呼び出し時読み）/`qss.py`（view から色構文排除）/`apply.py`（Qt 隔離・`build_main_window` 配線）を新設し散在ハードコード色を現状値のまま全置換。凍結検証=撮影/比較スクリプト（`scripts/capture_ui_screenshots.py`/`compare_screenshots.py`・QSettings 隔離・決定的 fixture）でピクセル前後比較 5/5 完全一致＋デバッグテーマ（`--debug-theme` 全トークン相異値）で役割写像目視。AST ガード `tests/gui/test_theme_guard.py`（ratchet allowlist・sabotage 実証）常設。レビューで捕捉: 同値別トークン（drop_highlight vs palette[0]）はピクセル比較・値ベース assert とも盲目→値分岐テーマテストで直接実証（memory [[gui_freeze_tokenization_verification_pattern]]）。副次発見: `WA_StyledBackground` 欠如で QSS 背景/枠不描画の疑い（既存挙動・別チップ起票・増分3前に解消推奨）。**次: 増分2（エクスポータ/design cards/撮影カタログ拡張/docs/design.md/初回同期。先頭タスク=DARK 全域スナップショット）→ 増分3（クロム統一=QStyle スパイク・初回のみ構造作業）** | 設計 [spec](docs/superpowers/specs/2026-07-15-design-token-pipeline-design.md)・プラン [r1-freeze](docs/superpowers/plans/2026-07-15-design-tokens-r1-freeze.md) |
