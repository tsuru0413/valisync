...
def test_files_list_contains_basenames() -> None:
    ...
    k1 = app_vm.session._groups.add(SignalGroup((), Path("/path/to/data1.mf4").absolute(), "MDF4", datetime.now()))
    k2 = app_vm.session._groups.add(SignalGroup((), Path("/path/to/data2.csv").absolute(), "CSV", datetime.now()))
...
def test_refreshes_on_loaded_notification() -> None:
    ...
    k = app_vm.session._groups.add(SignalGroup((), Path("/path/to/data1.mf4").absolute(), "MDF4", datetime.now()))
