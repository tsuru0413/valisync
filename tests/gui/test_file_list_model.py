...
def test_row_count_matches_vm(qtbot: QtBot) -> None:
    app_vm = AppViewModel()
    k1 = app_vm.session._groups.add(SignalGroup((), Path("/a.mf4").absolute(), "MDF4", datetime.now()))
    k2 = app_vm.session._groups.add(SignalGroup((), Path("/b.csv").absolute(), "CSV", datetime.now()))
...
def test_data_returns_filenames(qtbot: QtBot) -> None:
    app_vm = AppViewModel()
    k = app_vm.session._groups.add(SignalGroup((), Path("/path/a.mf4").absolute(), "MDF4", datetime.now()))
...
def test_refreshes_on_vm_notification(qtbot: QtBot) -> None:
    ...
    # Simulate load
    k = app_vm.session._groups.add(SignalGroup((), Path("/new.mf4").absolute(), "MDF4", datetime.now()))
    app_vm._loaded_keys = [k]
