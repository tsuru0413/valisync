...
意図的な変更は **supersede**: ゴールデン **11 本すべて**を ``git rm`` して
再生成する (生成は決定的なので、意図した 1 本だけが diff に出る)。理由を
docs/design.md 決定履歴へ記録してから本スクリプトを走らせる。1 本だけを
``git rm`` しても ``main()`` の既存チェック (cases 全件を対象に existing を見る)
は残り 10 本を「既存あり」として拒否する — 全部消してからでないと再実行できない
(M2)。``--force`` は**意図的に用意しない** (用意した瞬間に上の抑止が効かなくなる)。
"""
...
def main(
    ...
    if existing:
        ...
        print(
            "意図的に更新する場合は、ゴールデン 11 本すべてを `git rm` して "
            "理由を docs/design.md 決定履歴へ記録してから再実行してください "
            "(1 本だけ削除しても残りが「既存あり」で拒否されます・M2)。"
        )
        return 1
