...
def shadowed_leaf(
    ...
    **必ず**優先する (E-3 の test-lock 済み意図的仕様)。負けた側の列は列キーでは
    ...
    元々その次点にも負けているので、**リーフ名一致の場合は**実チャンネル 1 本が
    隠す列は 1 本だけ (= 衝突ペアあたり診断も 1 件)。

    本関数は spec §3.2 のリーフ限定スコープであり、実チャンネル名が配列の
    **中間ノード** (途中の親) に一致する配置は対象外 — その場合は部分木ごと
    到達不能になっても検出されない (既知の限界。§2 非スコープに defer 記録)。
