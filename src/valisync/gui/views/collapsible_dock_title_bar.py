...
class CollapsibleDockTitleBar(QWidget):
    ...
    def __init__(
        ...
        self._float_button.setIcon(icons.icon("float_dock"))
        # D-3 §2.5: iconSize を明示 16px 指定 — icon-only 化で minimumSizeHint が
        # ~22px 級まで下がり、直下の 24px 保証 (UX-38) とのマージンがフレーク圏に
        # 入るのを避ける (実測済み)。
        self._float_button.setIconSize(QSize(16, 16))
        self._float_button.setToolTip("フロート")
        # UX-38: 当たり判定の高さを 24px へ (icon-only の自然高さも 24px 未満)。
        ...
        self._close_button.setIcon(icons.icon("close"))
        self._close_button.setIconSize(QSize(16, 16))  # D-3 §2.5: 同上。
