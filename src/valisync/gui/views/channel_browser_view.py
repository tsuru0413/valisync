...
# Task 5 (#14 拡張・spec §1.5): QTreeView.sizeHint() は QAbstractScrollArea の
# 汎用既定値 QSize(256, 192) をそのまま返す実装で、列幅・resizeMode・行内容の
# いずれにも依存しない (Interactive 化して resizeSection で詰めても sizeHint
# は変わらないことを実測で確認済み)。この 256px は tree の「実 minimum」
# (minimumSizeHint は元々遥かに小さい) ではなく単なる Qt 側のプレースホルダ
...
class _ChannelTree(QTreeView):
    """sizeHint() の width 成分だけを差し替える薄い QTreeView 派生。
...
class ChannelBrowserView(QWidget):
    ...
    def __init__(self, vm: ChannelBrowserVM, parent: QWidget | None = None) -> None:
        ...
        self.tree = _ChannelTree(self)
        self.tree.setModel(self.model)
        # Task 5: elide long names instead of demanding their full width
        # (Qt's own default is already ElideRight -- set explicitly so the
        # behavior does not silently depend on an undocumented default).
        ...
        # Task 5: explicit, environment-independent floor for how far either
        # column can be dragged (Qt's own default is font/DPI-derived, ~16-23px).
        header.setMinimumSectionSize(_TREE_MIN_SECTION_SIZE)
